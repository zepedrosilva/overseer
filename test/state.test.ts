...
describe('State Store & Persistence', () => {
  ...
  describe('saveState and loadState', () => {
    ...
    it('persists and restores PRs and workers correctly', () => {
      ...
      const pr1 = createMockPR({ owner: 'MewsSystems', repo: 'billing', number: 142 }, 'Ready');
      const pr2 = createMockPR({ owner: 'MewsSystems', repo: 'meridian', number: 88 }, 'ChangesRequested');
  ...
  describe('Mutation & Query Helpers', () => {
    ...
    it('manages repo-specific agents and available agent lists', () => {
      const state = createEmptyState();
      expect(getRepoAgent(state, { owner: 'MewsSystems', repo: 'billing' })).toBe('claude');

      setRepoAgent(state, { owner: 'MewsSystems', repo: 'billing' }, 'agy');
      expect(getRepoAgent(state, { owner: 'MewsSystems', repo: 'billing' })).toBe('agy');
