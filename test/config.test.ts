...
describe('Config & Defaults Engine', () => {
  ...
  describe('Built-in Presets and Defaults', () => {
    ...
    it('contains built-in agent presets', () => {
      ...
      expect(BUILTIN_AGENT_PRESETS.moxly).toBeDefined();
    });
    ...
    it('resolves agent definitions for built-in presets and custom commands', () => {
      ...
      const moxlyDef = getAgentDefinition('moxly');
      expect(moxlyDef.command).toContain('@moxly');
  ...
  describe('loadConfig with State Integration', () => {
    it('loads state-backed configuration seamlessly', () => {
      ...
      setRepoAgent(state, { owner: 'MewsSystems', repo: 'billing' }, 'moxly');
  ...
  describe('getRepoAgent', () => {
    ...
    it('returns repo-specific agent when defined', () => {
      const customConfig = {
        ...DEFAULT_CONFIG,
        repos: [{ url: 'https://github.com/MewsSystems/billing', agent: 'agy' }],
      };
      const agent = getRepoAgent({ owner: 'MewsSystems', repo: 'billing' }, customConfig);
