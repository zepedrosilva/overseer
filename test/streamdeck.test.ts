...
describe('Stream Deck HTTP & SSE Server', () => {
  ...
  beforeEach(() => {
    ...
    state.repos = [{ owner: 'MewsSystems', repo: 'billing', url: 'https://github.com/MewsSystems/billing', agent: 'claude' }];
  ...
  it('serves GET /pr/:owner/:repo/:number with PR details', async () => {
    ...
    const res = await makeRequest('GET', '/pr/MewsSystems/billing/142', serverPort);
    expect(res.status).toBe(200);
    expect(res.parsed.id).toBe('MewsSystems/billing#142');
    ...
    const notFound = await makeRequest('GET', '/pr/MewsSystems/billing/999', serverPort);
